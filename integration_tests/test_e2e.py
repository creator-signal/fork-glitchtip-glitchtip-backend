...
def register_and_login():
    ...
    if resp.status_code == 409: # Conflict, already exists?
        ...
        update_csrf()
        resp = session.post(f"{BASE_URL}/_allauth/browser/v1/auth/login", json=login_data)
...
def create_team(org_slug):
    print("Creating team...")
    update_csrf()
    resp = session.post(f"{BASE_URL}/api/0/organizations/{org_slug}/teams/", json={"slug": TEAM_NAME})
    if resp.status_code == 201:
        return resp.json()
    elif resp.status_code == 400 and "already exists" in resp.text:
...
def create_project(org_slug, team_slug):
    ...
    update_csrf()
    resp = session.post(f"{BASE_URL}/api/0/teams/{org_slug}/{team_slug}/projects/", json={"name": PROJECT_NAME})
    if resp.status_code == 201:
        return resp.json()
    elif resp.status_code == 400 and "already exists" in resp.text:
        # Get existing
...
def send_event(dsn):
    ...
    try:
        output = subprocess.check_output(["sentry-cli", "send-event", "-m", "Hello GlitchTip E2E"], env=env)
...
def verify_event(org_slug, project_slug, event_id):
    ...
    while time.time() - start_time < 120:
        ...
        resp = session.get(f"{BASE_URL}/api/0/projects/{org_slug}/{project_slug}/issues/")
        if resp.status_code == 200:
            issues = resp.json()
            if issues:
