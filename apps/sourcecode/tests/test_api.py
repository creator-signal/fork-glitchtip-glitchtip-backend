...
class SourceCodeAPITestCase(GlitchTestCase):
    ...
    async def test_assemble_missing_chunks(self):
    ...
    async def test_assemble_partial_chunks(self):
        """When some chunks exist, only missing ones are returned."""
        existing_checksum = "a" * 40
        missing_checksum = "b" * 40
        await baker.amake("files.FileBlob", checksum=existing_checksum)
        data = {
    ...
    async def test_assemble_all_chunks_present(self):
        """When all chunks exist, state is created with empty missingChunks."""
        chunk1 = "a" * 40
        chunk2 = "b" * 40
        await baker.amake("files.FileBlob", checksum=chunk1)
        await baker.amake("files.FileBlob", checksum=chunk2)
        version = "app@v1"
        await baker.amake(
